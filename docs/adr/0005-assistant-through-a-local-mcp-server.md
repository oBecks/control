...
- **Amended 2026-09-26: the Assistant also makes Automations, Groups and Scenes** ([ADR 0006](0006-automations-run-in-the-engine.md)). It can create, edit and delete them from a plain-language description, and a new one is active right away: approving the tool call is the confirmation, the same as for controlling a Device. Each one is marked in the app as made by the Assistant, and every Automation has its own on/off switch and a run history. Hotkeys too ([ADR 0007](0007-hotkeys-through-a-keyboard-hook-in-the-desktop-app.md)), under the same rule. Dashboards stay in the app, since arranging a screen is visual work. Everything else below still holds: no Scans, Links, Learning, Code Set probes, renaming or phone approvals.
- **Read and control only** (as first decided; see the amendment above). The Assistant lists Devices (name, uid, Category, state) and changes state: on/off, brightness, colour, AC mode and temperature, pressing a remote's button. Scenes join once they exist. Setup stays in the UI: Scan, Links, Learning and Code Set probes need someone at the device, and approving phones and renaming are the user's decisions.
